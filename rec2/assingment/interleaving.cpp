...
void printInterleaving(string m, string n, string output)
{
	if (m.size() == 0 || n.size() == 0)
	{
		if (m.size() == 0) cout << output + n << endl;
		else if (n.size() == 0) cout << output + m << endl;
		return;
	}
	printInterleaving(m.substr(1), n, output + m[0]);
	printInterleaving(m, n.substr(1), output + n[0]);
}
